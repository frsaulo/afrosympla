 ...
 * @ORM\Table(name="AfroSympla_event_date_ticket")
 */
class EventTicket {
     ...
     */
    private $seatingPlanSections;

    public function checkIfSeatIsInTicketReservation($section, $row, $seat) {
        foreach ($this->ticketreservations as $ticketReservation) {
            if (!$ticketReservation->isExpired()) {
                foreach ($ticketReservation->getOrderelement()->getReservedSeats() as $reservedSeat) {
    ...
    public function isSeatAlreadyReservedByIds($sectionId, $rowId, $seatNumber) {
        foreach ($this->orderelements as $orderElement) {
            if ($orderElement->getOrder()->getStatus() == 1) {
                foreach ($orderElement->getTickets() as $boughtTicket) {
                    if ($boughtTicket->getReservedSeat()['sectionId'] == $sectionId && $boughtTicket->getReservedSeat()['rowId'] == $rowId && $boughtTicket->getReservedSeat()['seatNumber'] == $seatNumber) {
                        return true;
                    }
                }
            }
    ...
    public function isSeatAlreadyReserved($section, $row, $seatNumber) {
        foreach ($this->orderelements as $orderElement) {
            if ($orderElement->getOrder()->getStatus() == 1) {
                foreach ($orderElement->getTickets() as $boughtTicket) {
                    if ($boughtTicket->getReservedSeat()['sectionId'] == $section['randomId'] && $boughtTicket->getReservedSeat()['rowId'] == $row['randomId'] && $boughtTicket->getReservedSeat()['seatNumber'] == $seatNumber['number']) {
                        return true;
                    }
                }
            }
    ...
    public function getEventTicketCartElementsByUserAndSection($user, $section) {
        $userCartElements = [];
    ...
    public function isSeatInCart($user, $section, $row, $seatNumber) {
        foreach ($this->cartelements as $cartElement) {
            if ($cartElement->getUser() == $user) {
                foreach ($cartElement->getReservedSeats() as $reservedSeat) {
    ...
    public function stringifySections() {
    ...
    public function getValidTicketReservationsQuantitySum($user = null, $notUser = null) {
        $sum = 0;
        foreach ($this->ticketreservations as $ticketreservation) {
            if (!$ticketreservation->isExpired() && ($user == null || $ticketreservation->getUser() == $user) && ($notUser == null || $notUser !== $ticketreservation->getUser())) {
    ...
    public function getTicketsLeftCount($countValidTicketReservations = true, $notUser = null) {
        if ($countValidTicketReservations) {
            return $this->quantity - $this->getOrderElementsQuantitySum() - $this->getValidTicketReservationsQuantitySum(null, $notUser);
        } else {
    ...
    public function getMaxTicketsForSaleCount($user) {
        ...
        if ($this->ticketsperattendee) {
            if ($this->getTicketsLeftCount() >= $this->ticketsperattendee) {
                $ticketsForSaleCount = $this->ticketsperattendee - $this->getOrderElementsQuantitySum(1, $user);
            } else {
                $ticketsForSaleCount = $this->getTicketsLeftCount() - $this->ticketsperattendee - $this->getOrderElementsQuantitySum(1, $user);
            }
        } else {
            $ticketsForSaleCount = $this->quantity - $this->getOrderElementsQuantitySum();
        }
        if ($ticketsForSaleCount < 0) {
            $ticketsForSaleCount = 0;
        }
        return $ticketsForSaleCount;
    }
...
// set the owning side to null (unless already changed)
    ...
    public function setSeatingPlanSections(?array $seatingPlanSections): self {
